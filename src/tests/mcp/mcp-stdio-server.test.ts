...
jest.mock('../../services/memory.service', () => {
  const mockInstance = {
    ...
    exportMemoryBank: jest.fn().mockResolvedValue({
...
describe('MemoryMcpServer', () => {
  ...
  describe('Memory Bank Operations', () => {
    ...
    test('POST /mcp/tools/init-memory-bank should return 400 if repository is missing', async () => {
      const response = await request(app).post('/mcp/tools/init-memory-bank').send({});
  ...
  describe('Context Operations', () => {
    test("POST /mcp/tools/get-context with latest=true should get today's context", async () => {
      ...
      expect((await MemoryService.getInstance()).getTodayContext).toHaveBeenCalledWith(
        'test-repo',
        'main',
      );
    });
    ...
    test('POST /mcp/tools/get-context with latest=false should get latest contexts', async () => {
      const response = await request(app).post('/mcp/tools/get-context').send({
        repository: 'test-repo',
        branch: 'main',
        latest: false,
        limit: 5,
      ...
      expect((await MemoryService.getInstance()).getLatestContexts).toHaveBeenCalledWith(
        'test-repo',
        5,
        'main',
      );
    ...
    test('POST /mcp/tools/update-context should update context', async () => {
      const response = await request(app).post('/mcp/tools/update-context').send({
        repository: 'test-repo',
  ...
  describe('Export/Import Operations', () => {
    test('POST /mcp/tools/export-memory-bank should export a memory bank', async () => {
      ...
      expect((await MemoryService.getInstance()).exportMemoryBank).toHaveBeenCalledWith(
        'test-repo',
        'main',
      );
    });
    ...
    test('POST /mcp/tools/import-memory-bank should import memory bank content', async () => {
      const response = await request(app).post('/mcp/tools/import-memory-bank').send({
        repository: 'test-repo',
        type: 'metadata',
        id: 'meta',
