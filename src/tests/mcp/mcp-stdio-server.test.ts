...
describe("MemoryMcpServer", () => {
  ...
  describe("Memory Bank Operations", () => {
    test("POST /mcp/tools/init-memory-bank should initialize a memory bank", async () => {
      const response = await request(app)
        ...
        .send({ repository: "test-repo", branch: "main" });
      ...
      ).toHaveBeenCalledWith("test-repo", "main");
    });
  ...
  describe("Metadata Operations", () => {
    ...
    test("POST /mcp/tools/update-metadata should update metadata", async () => {
      ...
      expect(response.body.metadata).toBeDefined();
      expect(
        (await MemoryService.getInstance()).updateMetadata
      ).toHaveBeenCalledWith("test-repo", { project: { name: "UpdatedProject" } }, "main");
    });
  });
  ...
  describe("Context Operations", () => {
    test("POST /mcp/tools/get-context with latest=true should get today's context", async () => {
      const response = await request(app)
        ...
        .send({ repository: "test-repo", branch: "main", latest: true });

      expect(response.status).toBe(200);
      expect(response.body.success).toBe(true);
      expect(response.body.context).toHaveLength(1);
      expect(
        (await MemoryService.getInstance()).getTodayContext
      ).toHaveBeenCalledWith("test-repo", "main");
    });
    ...
    test("POST /mcp/tools/get-context with latest=false should get latest contexts", async () => {
      ...
      expect(
        (await MemoryService.getInstance()).getLatestContexts
      ).toHaveBeenCalledWith("test-repo", 5, "main");
    });
    ...
    test("POST /mcp/tools/update-context should update context", async () => {
      ...
      expect(
        (await MemoryService.getInstance()).updateTodayContext
      ).toHaveBeenCalled();
    });
  });
  ...
  describe("Component Operations", () => {
    test("POST /mcp/tools/add-component should add a component", async () => {
      ...
      expect(
        (await MemoryService.getInstance()).upsertComponent
      ).toHaveBeenCalled();
    });
  });
  ...
  describe("Decision Operations", () => {
    test("POST /mcp/tools/add-decision should add a decision", async () => {
      const response = await request(app).post("/mcp/tools/add-decision").send({
        repository: "test-repo",
        id: "dec-test",
        name: "TestDecision",
      ...
      expect(
        (await MemoryService.getInstance()).upsertDecision
      ).toHaveBeenCalled();
    });
  });
  ...
  describe("Export/Import Operations", () => {
    test("POST /mcp/tools/export-memory-bank should export a memory bank", async () => {
      ...
      expect(
        (await MemoryService.getInstance()).exportMemoryBank
      ).toHaveBeenCalledWith("test-repo", "main");
    });
    ...
    test("POST /mcp/tools/import-memory-bank should import memory bank content", async () => {
      ...
      expect(response.body.success).toBe(true);
      expect(
        (await MemoryService.getInstance()).importMemoryBank
      ).toHaveBeenCalledWith("test-repo", "yaml content", "metadata", "meta", "main");
    });
