      ...
      if (stats.isDirectory()) {
        ...
        for (const [filePath, content] of Object.entries(files)) {
          ...
          if (type && id) {
            ...
            await memoryService.importMemoryBank(repository, content, memoryType, id);
        ...
        if (yamlType) {
          // Ensure type is a valid MemoryType
          const memoryType = yamlType as 'metadata' | 'context' | 'component' | 'decision' | 'rule';
          await memoryService.importMemoryBank(repository, content, memoryType, fileNameNoExt);
...
program
  ...
  .action(async (repository: string, id: string, options) => {
    try {
      ...
      await memoryService.upsertComponent(repository, id, component);
...
program
  ...
  .action(async (repository: string, id: string, options) => {
    try {
      ...
      await memoryService.upsertDecision(repository, id, decision);
...
program
  ...
  .action(async (repository: string, id: string, options) => {
    try {
      ...
      await memoryService.upsertRule(repository, id, rule);
