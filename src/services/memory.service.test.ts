  ...
  ContextRepository: {
    getInstance: jest.fn().mockResolvedValue({
      ...
      getLatestContexts: jest.fn().mockImplementation(async (repoId, limit = 10) => {
        return contextStore.filter((c) => c.repository === repoId).slice(0, limit);
      }),
      upsertContext: jest.fn().mockImplementation(async (context) => {
        const existingIndex = contextStore.findIndex(
          (c) => c.repository === context.repository && c.yaml_id === context.yaml_id,
        );
    ...
    getInstance: jest.fn().mockResolvedValue({
      upsertComponent: jest.fn().mockImplementation(async (component) => {
        const existingIndex = componentStore.findIndex(
          (c) => c.repository === component.repository && c.yaml_id === component.yaml_id,
        );
      ...
      getActiveComponents: jest.fn().mockImplementation(async (repoId) => {
        return componentStore.filter((c) => c.repository === repoId && c.status === 'active');
      }),
    }),
  },
}));
    ...
    getInstance: jest.fn().mockResolvedValue({
      upsertDecision: jest.fn().mockImplementation(async (decision) => {
        const existingIndex = decisionStore.findIndex(
          (d) => d.repository === decision.repository && d.yaml_id === decision.yaml_id,
        );
  ...
  RuleRepository: {
    getInstance: jest.fn().mockResolvedValue({
      ...
      getActiveRules: jest.fn().mockImplementation(async (repoId) => {
        return ruleStore.filter((r) => r.repository === repoId && r.status === 'active');
      }),
    }),
  },
}));
