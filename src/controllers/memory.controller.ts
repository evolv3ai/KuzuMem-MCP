...
export class MemoryController {
  ...
  private asyncHandler = (
    fn: (req: Request, res: Response, next?: any) => Promise<void>
  ) => {
   ...
   */
  initMemoryBank = this.asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      try {
        ...
        res
          .status(200)
          .json({ message: "Memory bank initialized successfully" });
      } catch (error) {
   ...
   */
  updateMetadata = this.asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      try {
        const { repository } = req.params;
   ...
   * Get today's context
   */
  getTodayContext = this.asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      try {
        const { repository } = req.params;
   ...
   */
  updateTodayContext = this.asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      try {
        const { repository } = req.params;
   ...
   * Get latest contexts
   */
  getLatestContexts = this.asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      try {
        const { repository } = req.params;
   ...
   */
  upsertComponent = this.asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      try {
        ...
        if (!repository || !id) {
          res
            .status(400)
            .json({ error: "Repository name and component ID are required" });
          return;
        }
        ...
        if (!updatedComponent) {
          res
            .status(404)
            .json({ error: "Failed to create or update component" });
          return;
        }
   ...
   */
  getActiveComponents = this.asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      try {
        const { repository } = req.params;
   ...
   */
  upsertDecision = this.asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      try {
        const { repository, id } = req.params;
        ...
        if (!repository || !id) {
          res
            .status(400)
            .json({ error: "Repository name and decision ID are required" });
          return;
        }

        const result = decisionSchema
          .omit({ yaml_id: true })
          .safeParse(decision);
        ...
        if (!updatedDecision) {
          res
            .status(404)
            .json({ error: "Failed to create or update decision" });
          return;
        }
  ...
  getDecisionsByDateRange = this.asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      try {
        ...
        res
          .status(500)
          .json({ error: "Failed to get decisions by date range" });
      }
    }
  );
    ...
    async (req: Request, res: Response): Promise<void> => {
      try {
        ...
        if (!repository || !id) {
          res
            .status(400)
            .json({ error: "Repository name and rule ID are required" });
          return;
        }
   ...
   * Get active rules
   */
  getActiveRules = this.asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      try {
        const { repository } = req.params;
   ...
   * Export memory bank
   */
  exportMemoryBank = this.asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      try {
        const { repository } = req.params;
   ...
   * Import memory bank
   */
  importMemoryBank = this.asyncHandler(
    async (req: Request, res: Response): Promise<void> => {
      try {
        const { repository } = req.params;
